...
「比奇堡热心市民」的个人博客，基于 Hexo 静态站点生成器，使用自定义主题 `citizen`（经典文学风格：暖色纸张背景、衬线字体、双栏响应式布局）。站点语言为中文（zh-CN）。
...
- `blog/` 已提交到 git；部署通过 GitHub Actions（`.github/workflows/deploy.yml`），push 到 `main` 自动构建并发布到 GitHub Pages
...
- 主题实现了 `index`/`post`/`page`/`category`/`tag`/`archive`/`search` 等页面布局（`layout/` 目录），分类/标签/归档/关于/搜索页面均可正常渲染
